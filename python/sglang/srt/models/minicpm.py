...
class MiniCPMAttention(nn.Module):
    ...
    def forward(
        ...
        if forward_batch.sparse_16_loc is not None:
            self.compress_k1_len += len(forward_batch.sparse_16_loc)
        if forward_batch.sparse_64_loc is not None:
            self.compress_k2_len += len(forward_batch.sparse_64_loc)
        ...
        if self.layer_id == 0 and q.shape[0] == 8192:
            attn_output = self._sparse_attn_forward(q.reshape(1, 8192, 32, 128), k.reshape(1, 8192, 2, 128), v.reshape(1, 8192, 2, 128), 8192)
    ...
    def _sparse_attn_forward(
            self, query_states, key_states, value_states, query_length, dropout=0.0, softmax_scale=None, no_rope_param=None, past_key_value=None
        ):
