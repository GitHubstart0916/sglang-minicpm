...
class FlashAttentionBackend(AttentionBackend):
    ...
    def forward_extend(
        ...
        if q.shape[0] >= 8192:
            ...
            for i in range(bs):
                token_to_bs[metadata.cu_seqlens_q[i] : metadata.cu_seqlens_q[i + 1]] = i
            ...
            for i in range(bs):
                token_to_bs[metadata.cu_seqlens_q[i] : metadata.cu_seqlens_q[i + 1]] = torch.tensor(
                    [(i + 1) for i in range(seqlen_q_as_param[i].item())], dtype=token_to_bs.dtype, device=token_to_bs.device)
